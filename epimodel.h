/* class EpiModel
 *
 * This class models coronavirus transmission among people in a
 * synthetic population. The model is based on the influenza
 * model described in the paper "FluTE, a publicly available 
 * stochastic influenza epidemic simulation model," available at 
 * http://www.ploscompbiol.org/doi/pcbi.1000656
 * Transmission parameters were altered to reflect the longer
 * serial interval of coronavirus compared to influenza.
 */
...
struct Person {
  ...
  friend inline bool isInfectiousWorker(const Person &p) { return (p.status&INFECTED && p.iday>=0 && p.nDayComm!=p.nHomeComm && (p.age==2 || p.age==3)); }
  ...
  friend inline bool isIsolated(const Person &p) { return p.vbits&ISOLATED; }
  friend inline void setQuarantined(Person &p) {p.vbits|=ISQUARANTINED;}
  friend inline void setIsolated(Person &p) {p.vbits|=ISOLATED;}
  ...
  friend inline bool isWearingMask(const Person &p) { return p.vbits&MASK; }
  friend inline void setWearingMask(Person &p) {p.vbits|=MASK;}
  friend inline void clearWearingMask(Person &p) {p.vbits&=(~MASK);}
  friend inline void clearQuarantined(Person &p) {p.vbits&=(~ISQUARANTINED);}
  friend inline void clearIsolated(Person &p) {p.vbits&=(~ISOLATED);}
...
struct Tract {
  ...
  friend inline bool isWearingMask(const Tract &t) { return t.status&TRACTMASK; }
  ...
  friend inline void setWearingMask(Tract &t) {t.status|=TRACTMASK;}
  friend inline void clearWearingMask(Tract &t) {t.status&=~TRACTMASK;}
...
class EpiModel {
 public:
  ...
  Network net= Network("name", Network::Undirected);
  //Network("name", Network::Undirected);
  //Network net;
  Percolation_Sim sim = Percolation_Sim(&net);
  ...
  virtual void commutenetwork(void);
  double fraction_to_migrate;
  int num_to_migrate;
  vector<Node*> nodes;
 ...
 protected:
  ...
  vector < unsigned int > nNumWorkFromHomePerDayHistory;// history of number of symptomatics (across all nodes)
  vector < unsigned int > nWorkInfectiousHistory;
  vector < unsigned int > nCommuteExposedHistory;
  vector < unsigned int > nCounterHistory;
  ...
  double fPregnantFraction[TAG];    // fraction of working-age adults who are pregnantest
  double fHighRiskFraction[TAG];    // fraction of individuals who are at high risk for mortality
  double fPTRestriction;
