...
EpiModelParameters::EpiModelParameters(const char *configname) {
  ...
  AVEs = 0.6;  // less likely to get infected
  AVEp = 0.6;  // less likely to become sick given infection
  AVEi = 0.6; // less likely to infect others

  //  nTriggerTime=-1; // not set
  nTriggerDay=-1; // not set
  nTriggerDelay=1;
  bTrigger=false;
  ...
  nQuarantineLength=21; // default 14-day quarantine
  ...
  if (!iss) {
	...
	if (line[0]=='#') {
	  ...
	  if (beta<0.0 || beta>1.0) {
	    cerr << "ERROR: symptomaticfraction must be between 0 and 1" << endl;
	    exit(-1);
	  }
	...
	} else if (param.compare("PTRestriction")==0) {
	    read_config_double(fPTRestriction, iss, "PTRestriction", 0.0, 1.0);
	...
	} else if (param.compare("cpcm")==0) {
	    read_config_double(fcpcm[0], iss, "cpcm", 0.0, 1.0);
	    read_config_double(fcpcm[1], iss, "cpcm", 0.0, 1.0);
	    read_config_double(fcpcm[2], iss, "cpcm", 0.0, 1.0);
	    read_config_double(fcpcm[3], iss, "cpcm", 0.0, 1.0);
	    read_config_double(fcpcm[4], iss, "cpcm", 0.0, 1.0);
	} else if (param.compare("cpnh")==0) {
	    read_config_double(fcpnh[0], iss, "cpnh", 0.0, 1.0);
	    read_config_double(fcpnh[1], iss, "cpnh", 0.0, 1.0);
	    read_config_double(fcpnh[2], iss, "cpnh", 0.0, 1.0);
	    read_config_double(fcpnh[3], iss, "cpnh", 0.0, 1.0);
	    read_config_double(fcpnh[4], iss, "cpnh", 0.0, 1.0);
	} else if (param.compare("cphcc")==0) {
	    read_config_double(fcphcc[0], iss, "cphcc", 0.0, 1.0);
	    read_config_double(fcphcc[1], iss, "cphcc", 0.0, 1.0);
	    read_config_double(fcphcc[2], iss, "cphcc", 0.0, 1.0);
	    read_config_double(fcphcc[3], iss, "cphcc", 0.0, 1.0);
	    read_config_double(fcphcc[4], iss, "cphcc", 0.0, 1.0);
	} else if (param.compare("cphca")==0) {
	    read_config_double(fcphca[0], iss, "cphca", 0.0, 1.0);
	    read_config_double(fcphca[1], iss, "cphca", 0.0, 1.0);
	    read_config_double(fcphca[2], iss, "cphca", 0.0, 1.0);
	    read_config_double(fcphca[3], iss, "cphca", 0.0, 1.0);
	    read_config_double(fcphca[4], iss, "cphca", 0.0, 1.0);
	...
	} else if (param.compare("mask")==0) {
	    read_config_double(fMaskCompliance, iss, "mask wear compliance probability", 0.0, 1.0);
	} else if (param.compare("maskdays")==0) {
	    read_config_int(nMaskDuration, iss, "mask wear duration");
	...
	} else if (param.compare("PTRestrictiondays")==0) {
	    read_config_int(nPTRestrictionDuration, iss, "PT Restriction duration");
